 ...
 */
(function ($) {
    $.fn.extendPagination = function (totalCount,showPage,limit,options) {
        var defaults = {
            //pageId:'',
            totalCount: '200',
            showPage: '10',
        ...
        var totalCount = parseInt(totalCount), showPage = parseInt(showPage),
            limit = parseInt(limit), totalPage = Math.ceil(totalCount / limit);
        if (totalPage > 0) {
            var html = [];
            ...
            if (totalPage <= showPage) {
                for (var i = 1; i <= totalPage; i++) {
                    if (i == 1) html.push(' <li class="active"><a href="javascript:void(0);">' + i + '</a></li>');/*每个a标签都加了点击事件*/
 ...
 * 页码栏居中*/
function pageCenter(){
    var $outer = $('.callBackPager');
    var $inner = $('.pagination');
    //
    var innerWidth = parseInt( $inner.css('width') );
    var outerWidth = parseInt( $outer.css('width') );
    var innerLeft = outerWidth / 2 - Math.ceil( innerWidth / 2 );
    $inner.css('left', innerLeft + 'px');
}